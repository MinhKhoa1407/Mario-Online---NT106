...
void CallLogoutAPI(const unsigned int score, const std::string localId, const std::string idToken) {
	...
	std::string url = "https://localhost:7244/api/Firebase/updateScore";
	std::string jsonData = "{"
				...
				switch(vOLDDir[stepID]) {
					...
					case eGAMEOVER: {
						if (!hasSentScore) {
							...
							CallLogoutAPI(score, localId, idToken);
							hasSentScore = true; 
						}
